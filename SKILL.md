...
- **我来审批**：默认审批策略，Codex 遇到需确认的操作会暂停，pane monitor 唤醒我，我判断批准或拒绝，大哥不需要介入

两种模式下，**中间过程（审批、迭代、修改）都由我自主处理，大哥只关心最终结果**。
...
如果中间发现**方向性问题**（任务理解有偏差、架构需要大改），则立即汇报大哥确认，不自行决定。
...
```
Codex 完成 turn ──→ on_complete.py ──→ 大哥收到 🔔 Telegram
                                   └──→ Agent 被唤醒（openclaw agent）

Codex 等审批 ───→ pane_monitor.sh ──→ 大哥收到 ⏸️ Telegram
                                   └──→ Agent 被唤醒（openclaw agent）
```
...
6. 通知目标通过环境变量 `CODEX_AGENT_CHAT_ID`、可选的 `CODEX_AGENT_ACCOUNT` 和 `CODEX_AGENT_NAME` 配置，也可直接修改脚本中的默认值
...
- [ ] 如使用多账号通道，环境变量 `CODEX_AGENT_ACCOUNT` 已设置（可选）
